...
export default function ReportsPage() {
    ...
    const exportToPDF = () => {
        ...
        doc.text(`Total Expenses: -${currency}${totalExpense}`, 14, 48);
        doc.text(`Net Balance: ${currency}${netSavings}`, 14, 56);

        const tableColumn = ["Description", "Category", "Type", "Date", "Amount"];
        const tableRows = expenses.map((t) => {
            const desc = t.title || t.description || "";
            const cat = t.category || "General";
            const type = t.type || "Expense";
            const date = t.date ? new Date(t.date).toLocaleDateString() : "";
            const amount = (type.toLowerCase() === "income" ? `+${currency}` : `-${currency}`) + (t.amount || 0);
            return [desc, cat, type, date, amount];
        });
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={exportToCSV} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-medium transition flex items-center gap-2 shadow-lg shadow-emerald-600/20">
                        <Download className="w-4 h-4" /> Export CSV
                    </button>
                    <button onClick={exportToPDF} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-medium transition flex items-center gap-2 shadow-lg shadow-indigo-600/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-[#121624] p-6 rounded-2xl border border-gray-800 space-y-2">
                    ...
                    <div className="text-3xl font-bold text-emerald-400">+{currency}{totalIncome.toLocaleString()}</div>
                    <p className="text-xs text-gray-500">All recorded inflows</p>
                </div>
                <div className="bg-[#121624] p-6 rounded-2xl border border-gray-800 space-y-2">
                    <span className="text-sm text-gray-400">Total Expenses</span>
                    <div className="text-3xl font-bold text-rose-500">-{currency}{totalExpense.toLocaleString()}</div>
                    <p className="text-xs text-gray-500">All recorded outflows</p>
                </div>
                <div className="bg-[#121624] p-6 rounded-2xl border border-gray-800 space-y-2">
                    <span className="text-sm text-gray-400">Net Balance</span>
                    <div className="text-3xl font-bold text-indigo-400">{currency}{netSavings.toLocaleString()}</div>
